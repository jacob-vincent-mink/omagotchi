...
> Experimental schema-v2 migration for [Omarchy PR #8956](https://github.com/omacom/omarchy/pull/8956), not a schema-v1 installation update. Requires the opt-in runtime. Private storage, notifications, and sanitized system observation are optional permissions; audio playback is unavailable. Original artwork and sound assets remain unchanged for review, but this port does not play the sound assets. Existing screenshots are not new verification of this branch.
...
Evolutions are announced through the optional desktop-notification permission. Care average resets at
...
The secure POC has no sound provider. Its unused sound permission, controls, and calls have been removed; the plugin does not launch an audio player.
...
Code and sprites: MIT — see [LICENSE](LICENSE). Credits for removed sound assets remain in [CREDITS.md](CREDITS.md).
